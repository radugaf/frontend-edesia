...
import {
  editTodoElement,
  fetchTodoListData,
} from "../../Todo/redux/actions";
import todoCard from "../../Todo/types";
import ProfileToDo from "./ProfileTodo";

const editTodoElementData = ({ todoElements, editTodoElementAction }) => (
  e
) => {
  const todoId = e.target.id;
  const elementData = todoElements.find(
    (item) => Number(item.data.id) === Number(todoId)
  ).data;
...
const ProfileTasks = ({
  ...
  useEffect(() => {
    if (JSON.stringify(todoElements) !== JSON.stringify(prevTodoElements)) {
      ...
      setNoArchivedTodoElements(
        filteredData.filter((item) => !item.data.isArchived)
      );
      setArchivedTodoElements(
        filteredData.filter((item) => item.data.isArchived)
      );
...
const mapStateToProps = (state) => {
  const todoElements =
    state.todo &&
    state.todo.data &&
    state.todo.data.elements &&
    state.todo.data.elements.length > 0
      ? [...state.todo.data.elements]
      : [];
  return {
    todoElements,
