          ...
          <ul className="card-row order-history-wrapper">
            {Object.keys(orders) &&
              Object.keys(orders).map((key) => (
                <li className="order-history-supplier">
                  <span className="color-blue">
                    <i class="fas fa-cash-register color-blue margin-right-10"></i>
                    {key}
                    <i
                      onClick={() => setSupplierOne(key)}
                      class="istoric-comenzi-supplier-dropdown color-blue fas fa-chevron-down"
                    ></i>
                  </span>
                  {supplierOne &&
                    orders &&
                      ...
                      <>
                        <form>
                          <ol className="order-history-date-wrapper">
                            <li className="order-history-date">
                              <span>
                                <i class="margin-right-10 color-green far fa-calendar-alt"></i>
                                Data {key}
                                <i
                                  onClick={() => setDateOne(key)}
                                  class="istoric-comenzi-date-dropdown color-green fas fa-chevron-down"
                                ></i>
                              </span>
                              {dateOne &&
                                orders[supplierOne] &&
                                orders[supplierOne][key] &&
                                orders[supplierOne][key].map((product) => (
                                  <ul className="order-history-table-wrapper">
                                    <li>
                                      <table className="order-history-table">
                                        <tr>
                                          <th className="td-vertical-center">
                                            Select
                                          </th>
                                          <th>Poza</th>
                                          <th>Nume Produs</th>
                                        ...
                                        <tr>
                                          ...
                                          <td>
                                            ...
                                            <img
                                              width="10%"
                                              src={`${URL}${product.product_image_url}`}
                                            ></img>
                                          </td>
                                        ...
                                        <tr>
                                          <td
                                            colSpan="6"
                                            className="istoric-comenzi-wrapper"
                                          >
                                            <button
                                              className="istoric-comenzi-button"
                                              onClick={acceptOrder}
                                            >
                                              Mark as delivered
                                            </button>
